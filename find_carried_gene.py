...
def get_args():
    ...
    parser.add_argument('-s', '--synaty_file', default= "Ner_vs_Eso.new.collinearity_4_circos")
    ...
    return input_name, output_name, gff_name, syn_name
...
def find_loci(ltr_list, gff_list, syn_list):
...
def main():
    input_name, output_name, gff_name, syn_name = get_args()
    ltr_list = get_ltr_gene(input_name)
    gff_list = get_gff(gff_name)
    syn_list = get_synaty(syn_name)
    print(gff_list)
    time_start = time.time()
    ltr_list = find_loci(ltr_list,gff_list,syn_list)
    time_stop = time.time()
    #print(ltr_list)

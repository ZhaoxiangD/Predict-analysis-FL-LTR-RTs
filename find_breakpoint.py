...
def list_process(breakpointlist, a_chrlist):
    breaklist = pd.DataFrame(columns=['chra','chrb1', 'chrb2','break_start', 'break_stop', 'reverse', 'LTR_num', 'LTR_len', 'LTR_percent', 'gypsy_num', 'gypsy_len', 'gypsy_percent', 'copia_num', 'copia_len', 'copia_percent', 'bel_num', 'bel_len','bel_percent', 'erv_num', 'erv_len', 'erv_percent'])
...
def get_breakltr(breakpointlist,faname,speciename):
    for i, rows in breakpointlist.iterrows():
        ...
        rows['LTR_num'] =  ltr_tbl.loc[0,2] 
        rows['LTR_len'] =  ltr_tbl.loc[0,3]
        rows['LTR_percent'] = ltr_tbl.loc[0,5]
        rows['bel_num'] = ltr_tbl.loc[1,1]
        rows['bel_len'] = ltr_tbl.loc[1,2]
        rows['bel_percent'] = ltr_tbl.loc[1,4]
        rows['copia_num'] = ltr_tbl.loc[2,1]
        rows['copia_len'] = ltr_tbl.loc[2,2]
        rows['copia_percent'] = ltr_tbl.loc[2,4]
        rows['gypsy_num'] = ltr_tbl.loc[3,1]
        rows['gypsy_len'] = ltr_tbl.loc[3,2]
        rows['gypsy_percent'] = ltr_tbl.loc[3,4]
        rows['erv_num'] = ltr_tbl.loc[4,1]
        rows['erv_len'] = ltr_tbl.loc[4,2]
        rows['erv_percent'] = ltr_tbl.loc[4,4]

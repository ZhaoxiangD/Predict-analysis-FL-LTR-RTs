...
def get_args():
    parser = argparse.ArgumentParser(description="find LTR foward gene")
    parser.add_argument('-i', '--input_file', default='hmmresult.xls')
    parser.add_argument('-o', '--output_file', default="foward_gene.csv")
...
def get_ltr_gene(input_name):
    ...
    ltr_list[['foward_gene_type','foward_gene_start','foward_gene_end','foward_gene_score',
    'foward_gene_strand','foward_gene_phase','foward_gene_attributes']] = 'No gene exist in the chromosome'
    return ltr_list
...
def find_loci(ltr_list, gff_list, syn_list, bp):
    for i, rows in ltr_list.iterrows():
        ...
        if mid == 0:
            continue
        if ltr_foward_start < int(gff_list_filt.loc[mid,3]) and int(rows['ltr_start']) > int(gff_list_filt.loc[mid,4]):
            syn_result = syn_filt(rows['chr_code'], gff_list_filt.loc[mid,3], gff_list_filt.loc[mid,4], syn_list)
            if syn_result:
                ltr_list.loc[i,'foward_gene_type'] = gff_list_filt.loc[mid,2]
                ltr_list.loc[i,'foward_gene_start'] = gff_list_filt.loc[mid,3]
                ltr_list.loc[i,'foward_gene_end'] = gff_list_filt.loc[mid,4]
                ltr_list.loc[i,'foward_gene_score'] = gff_list_filt.loc[mid,5]
                ltr_list.loc[i,'foward_gene_strand'] = gff_list_filt.loc[mid,6]
                ltr_list.loc[i,'foward_gene_phase'] = gff_list_filt.loc[mid,7]
                ltr_list.loc[i,'foward_gene_attributes'] = gff_list_filt.loc[mid,8]
        elif int(rows['ltr_start']) < int(gff_list_filt.loc[mid,3]):
            ...
            for gi, grows in gff_list_filt_half.iterrows():
                if ltr_foward_start < int(grows[3]) and int(rows['ltr_start']) > grows[4]:
            ...
            ltr_list.loc[i,'foward_gene_type'] = "/".join(g_type)
            ltr_list.loc[i,'foward_gene_start'] = "/".join(g_start)
            ltr_list.loc[i,'foward_gene_end'] = "/".join(g_end)
            ltr_list.loc[i,'foward_gene_score'] = "/".join(g_score)
            ltr_list.loc[i,'foward_gene_strand'] = "/".join(g_strand)
            ltr_list.loc[i,'foward_gene_phase'] = "/".join(g_phase)
            ltr_list.loc[i,'foward_gene_attributes'] = "/".join(g_attri)
        elif ltr_foward_start > int(gff_list_filt.loc[mid,4]):
            g_type, g_start, g_end, g_score, g_strand, g_phase, g_attri = ['NA'],['NA'],['NA'],['NA'],['NA'],['NA'],['NA']
            gff_list_filt_half = gff_list_filt.loc[mid:, :]
            gff_list_filt_half.reset_index(drop = True, inplace = True)
            for gi, grows in gff_list_filt_half.iterrows():

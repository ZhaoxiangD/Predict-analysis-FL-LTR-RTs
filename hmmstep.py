...
*gypsy/copia/erv/belpao/unclass*
...
def getfgenesh(xlsdict):
    ...
    '''
    copia = 'False_copia' #superfamily default exisistence is false. those exist will change into true
    gypsy = 'False_gypsy'
    belpao = 'False_belpao'
    erv = 'False_erv'
    unclass = 'False_unclass'
    for num in range(len(xlsdict)): # categorlize ltr's different superfamily fasta file
    ...
    familylist = [gypsy, copia, belpao, erv, unclass] #represent superfamily exisistence
    for familyname in range(len(familylist)): #creating aa sequence file from gene sequence; step 1
        if familylist[familyname].split('_')[0] == 'True': #base on the existence 
    ...
    for name in range(len(familylist)):
        if familylist[name].split('_')[0] == 'True' and familylist[name].split('_')[1] != 'unclassified':
            try:
                ...
                continue
        else: #scan unclass aa file with each superfamily aa sequence library
...
def unclassdom_process(unclass_gypsy_domdict, unclass_copia_domdict, unclass_belpao_domdict, unclass_erv_domdict, selected_domdict):
    '''
    base on the unclass aa sequence Evalue with library's sequence, estimate the family it belongs to
    '''
    ...
    '''
    select lowest Evalue in one family's file
    '''
    ...
    if unclass_copia_exist:
        ...
        for ci in range(1,len(unclass_copia_domdict)):
            if unclass_copia_domdict[ci]['domID'] != unclass_copia_domdict[ci-1]['domID']:
                selected_copia_domdict.append(unclass_copia_domdict[ci])
    else:
    ...
    '''
    compare one sequence among different family's file and select the one has lowest Evalue
    '''
...
def domdict_process(domdict):
    '''
    ordinary sequence selection
    '''
    selected_domdict = []
    for family in range(len(domdict)):
        if len(domdict[family]) == 0:
            domdict[family].append({})
            domdict[family][0]['domID'] = 'none'
            domdict[family][0]['unclass'] = 'False'
...
def getaafile(alidict):
    '''
    use samtools to cut RT domain aa sequence
    '''
